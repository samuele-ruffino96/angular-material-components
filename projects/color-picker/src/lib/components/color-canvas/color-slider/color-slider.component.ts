...
export class NgxMatColorSliderComponent
  ...
  implements AfterViewInit
{
  ...
  public fillGradient() {
    if (!this.ctx) {
      throw new Error(
        "Could not fill gradient. Canvas context is not initialized",
      );
    }
    if (!this.height) {
      throw new Error("Could not fill gradient. Height is not defined");
    }
  ...
  public redrawIndicator(x: number, y: number) {
    if (!this.ctx) {
      throw new Error(
        "Could not redraw indicator. Canvas context is not initialized",
      );
    }
    this.ctx.beginPath();
  ...
  public changeColor(e: MouseEvent) {
    if (!this.ctx) {
      throw new Error(
        "Could not change color. Canvas context is not initialized",
      );
    }
    this.x = e.offsetX;
    this.y = e.offsetY;
