...
export class NgxMatMultiYearViewComponent<D>
  implements AfterContentInit, OnDestroy
{
  ...
  set activeDate(value: D) {
    const oldActiveDate: D = this._activeDate;
    const validDate: D =
  ...
  _years: NgxMatCalendarCell<D>[][] | null = null;
  ...
  _focusActiveCell() {
    if (!this._matCalendarBody) {
      throw Error("Could not focus active cell as calendar body is missing.");
    }
  ...
  _focusActiveCellAfterViewChecked() {
    if (!this._matCalendarBody) {
      throw Error(
        "Could not focus active cell after the view has been checked as calendar body is missing.",
      );
    }
  ...
  private _createCellForYear(year: number): NgxMatCalendarCell<D> {
