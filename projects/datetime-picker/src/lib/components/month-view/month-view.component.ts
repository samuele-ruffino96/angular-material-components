...
import { NgxDateRange } from "../../utils/date-selection-model";
...
export class NgxMatMonthViewComponent<D>
  implements AfterContentInit, OnChanges, OnDestroy
{
  ...
  @Output() readonly dragEnded = new EventEmitter<
    NgxMatCalendarUserEvent<NgxDateRange<D> | null>
  >();
  ...
  @ViewChild(NgxMatCalendarBodyComponent)
  _matCalendarBody: NgxMatCalendarBodyComponent | null = null;

  /** The label for this month (e.g. "January 2017"). */
  _monthLabel: string | null = null;

  /** Grid of calendar cells representing the dates of the month. */
  _weeks: NgxMatCalendarCell<D>[][] | null = null;
  ...
  _focusActiveCell(movePreview?: boolean) {
    if (!this._matCalendarBody) {
      throw Error("Could not focus active cell. Calendar body is missing.");
    }
  ...
  _focusActiveCellAfterViewChecked() {
    if (!this._matCalendarBody) {
      throw Error(
        "Could not focus active cell after view has been checked. Calendar body is missing.",
      );
    }
  ...
  _previewChanged({
    ...
    if (!event) {
      throw Error("Could not update preview. Event is missing.");
    }
    if (this._rangeStrategy) {
      // We can assume that this will be a range, because preview
      // events aren't fired for single date selections.
      const value = cell?.rawValue || null;
  ...
  protected _dragEnded(event: NgxMatCalendarUserEvent<D | null>) {
    if (!event.event) {
      throw Error("Could not end drag. Event is missing.");
    }
  ...
  private _createWeekCells() {
    ...
    this._weeks = [[]];
    if (!this._firstWeekOffset) {
      throw Error("Could not create week cells. First week offset is missing.");
    }
    for (
