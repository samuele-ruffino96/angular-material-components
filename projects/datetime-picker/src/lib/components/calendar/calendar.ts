...
import { NgxMatMonthViewComponent } from "../month-view/month-view.component";
import {
  NgxMatMultiYearViewComponent,
  getActiveOffset,
  isSameMultiYearView,
  yearsPerPage,
} from "../multi-year-view/multi-year-view";
import { NgxMatYearViewComponent } from "../year-view/year-view";
...
export class NgxMatCalendarHeaderComponent<D> {
  constructor(
    private _intl: NgxMatDatepickerIntl,
    @Inject(forwardRef(() => NgxMatCalendarComponent))
    public calendar: NgxMatCalendarComponent<D>,
...
export class NgxMatCalendarComponent<D>
  implements AfterContentInit, AfterViewChecked, OnDestroy, OnChanges
{
  @HostBinding("class") class = "ngx-mat-calendar";
  ...
  @Output() readonly _userDragDrop = new EventEmitter<
    NgxMatCalendarUserEvent<NgxDateRange<D>>
  >();

  /** Reference to the current month view component. */
  @ViewChild(NgxMatMonthViewComponent)
  monthView: NgxMatMonthViewComponent<D> | null = null;

  /** Reference to the current year view component. */
  @ViewChild(NgxMatYearViewComponent)
  yearView: NgxMatYearViewComponent<D> | null = null;

  /** Reference to the current multi-year view component. */
  @ViewChild(NgxMatMultiYearViewComponent)
  multiYearView: NgxMatMultiYearViewComponent<D> | null = null;
   ...
   */
  get activeDate(): D {
    if (!this._clampedActiveDate) {
      throw Error(
        "Could not determine the active date. Active date is not set.",
      );
    }
  ...
  get currentView(): NgxMatCalendarView {
    if (!this._currentView) {
      throw Error(
        "Could not determine the current calendar view. Current view is not set.",
      );
    }
  ...
  private _currentView: NgxMatCalendarView | null = null;
  ...
  ngAfterContentInit() {
    this._calendarHeaderPortal = new ComponentPortal(
      this.headerComponent || NgxMatCalendarHeaderComponent,
    );
  ...
  private _getCurrentViewComponent():
    | NgxMatMonthViewComponent<D>
    | NgxMatYearViewComponent<D>
    | NgxMatMultiYearViewComponent<D> {
    // The return type is explicitly written as a union to ensure that the Closure compiler does
    // not optimize calls to _init(). Without the explicit return type, TypeScript narrows it to
    // only the first component type. See https://github.com/angular/components/issues/22996.
    const currentView = this.monthView || this.yearView || this.multiYearView;
    if (!currentView) {
      throw Error(
        "Could not find the current calendar view. All views are null.",
      );
    }
    return currentView;
  }
