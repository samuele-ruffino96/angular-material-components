  ...
  implements ControlValueAccessor, OnInit, OnChanges, OnDestroy
{
  ...
  @Input() defaultTime: number[] | null = null;
  ...
  private _onChange: (_: unknown) => unknown = () => {};
  private _onTouched = () => {};
  private _disabled: boolean = false;
  private _model: D | null = null;
  ...
  private _updateHourMinuteSecond() {
    if (!this._model) {
      throw new Error(
        "Could not update hour, minute and second. Model is null",
      );
    }
  ...
  private _getNextValueByProp(
    ...
    // Check if keyProp is "Hour", "Minute" or "Second"
    if (keyProp !== "Hour" && keyProp !== "Minute" && keyProp !== "Second") {
      throw new Error(
        "Could not get next value by property. Property ´names must be 'hour', 'minute' or 'second'",
      );
    }
  ...
  private _setDisableStates() {
    if (this.disabled) {
      ...
      if (this.disableMinute) {
        const minuteChildControl = this.form.get("minute");
        if (minuteChildControl) {
          minuteChildControl.disable();
        } else {
          console.error("Minute control is not found");
        }
        if (this.showSeconds) {
          const secondChildControl = this.form.get("second");
          if (secondChildControl) {
            secondChildControl.disable();
          } else {
            console.error("Second control is not found");
          }
        }
      }
    }
