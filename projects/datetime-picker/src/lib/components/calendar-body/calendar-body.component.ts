 ...
 * @docs-private
 */
export class NgxMatCalendarCell<D> {
  constructor(
    public value: number,
...
export class NgxMatCalendarBodyComponent<D = unknown>
  implements OnChanges, OnDestroy, AfterViewChecked
{
   ...
   */
  private _focusActiveCellAfterViewChecked: boolean = false;

  /** The label for the table. (e.g. "Jan 2017"). */
  @Input() label: string | null = null;

  /** The cells to display in the table. */
  @Input() rows: NgxMatCalendarCell<D>[][] | null = null;
  ...
  @Output() readonly selectedValueChange = new EventEmitter<
    NgxMatCalendarUserEvent<number>
  >();
  ...
  _cellClicked(cell: NgxMatCalendarCell<D>, event: MouseEvent): void {
  ...
  _emitActiveDateChange(cell: NgxMatCalendarCell<D>, event: FocusEvent): void {
    if (cell.enabled) {
  ...
  _isComparisonBridgeStart(value: number, rowIndex: number, colIndex: number) {
    ...
    if (!this.rows) {
      console.error(
        "Could not check if cell is a comparison bridge start. Rows are not defined.",
      );
      return false;
    }
    let previousCell: NgxMatCalendarCell<D> | undefined =
      this.rows[rowIndex][colIndex - 1];

    if (!previousCell) {
      const previousRow: NgxMatCalendarCell<D>[] = this.rows[rowIndex - 1];
  ...
  _isComparisonBridgeEnd(value: number, rowIndex: number, colIndex: number) {
    ...
    if (!this.rows) {
      console.error(
        "Could not check if cell is a comparison bridge end. Rows are not defined.",
      );
      return false;
    }
    let nextCell: NgxMatCalendarCell<D> | undefined =
      this.rows[rowIndex][colIndex + 1];

    if (!nextCell) {
      const nextRow: NgxMatCalendarCell<D>[] = this.rows[rowIndex + 1];
  ...
  private _enterHandler = (event: Event) => {
    ...
    if (event.target && this.isRange) {
      const cell: NgxMatCalendarCell<D> | null = this._getCellFromElement(
  ...
  private _touchmoveHandler = (event: TouchEvent) => {
    ...
    const cell: NgxMatCalendarCell<D> | null = target
  ...
  private _mousedownHandler = (event: Event) => {
    ...
    const cell: NgxMatCalendarCell<D> | null =
      event.target && this._getCellFromElement(event.target as HTMLElement);
    if (!cell || !this._isInRange(cell.value)) {
      console.error("Could not start drag sequence. Target is not a cell.");
      return;
    }
    let rawValue = cell.rawValue;

    if (rawValue === undefined) {
      console.warn("Could not start drag sequence. Cell has no raw value.");
      return;
    } else {
      rawValue = rawValue as D;
    }
  ...
  private _getCellFromElement(
    element: HTMLElement,
  ): NgxMatCalendarCell<D> | null {
    if (!this.rows) {
      throw Error("Could not get cell from element, because there are no rows");
    }
