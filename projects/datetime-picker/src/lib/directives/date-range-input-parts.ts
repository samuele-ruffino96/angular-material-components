...
import { _computeAriaAccessibleName } from "../utils/aria-accessible-name";
...
export interface NgxMatDateRangeInputParent<D> {
  ...
  _startInput: NgxMatDateRangeInputPartBase<D> | null;
  _endInput: NgxMatDateRangeInputPartBase<D> | null;
  ...
  providers: [
    {
      ...
      useExisting: NgxMatStartDateDirective,
      multi: true,
    },
    {
...
export class NgxMatStartDateDirective<D>
  ...
  implements CanUpdateErrorState
{
  ...
  override _onKeydown(event: KeyboardEvent) {
    ...
    if (
      ...
      if (endInput === null) {
        throw Error("Could not handle onKeydown event: end input is missing");
      }
...
@Directive({
  ...
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: NgxMatEndDateDirective,
      multi: true,
    },
    {
      provide: NG_VALIDATORS,
...
export class NgxMatEndDateDirective<D>
  extends _NgxMatDateRangeInputBase<D>
  implements CanUpdateErrorState
{
  ...
  override _onKeydown(event: KeyboardEvent) {
    ...
    const isLtr = this._dir?.value !== "rtl";
    if (startInput === null) {
      throw Error("Could not handle onKeydown event: start input is missing");
    }
